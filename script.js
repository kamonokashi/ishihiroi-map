      ...
      <ul class="mineral-list">
        ${found.map((mineral) => `
          <li class="mineral-item mineral-${escapeHtml(mineral.level)}">
            ...
            <a class="secondary-button mineral-detail-link" href="mineral.html?id=${encodeURIComponent(mineral.id)}" target="_blank" rel="opener">${text.details}</a>
          </li>
        `).join("")}
      </ul>
    </section>
...
function renderRockCard(rock) {
  ...
  return `
    <article class="rock-card rock-card-${escapeHtml(rock.level)}" data-rock="${escapeHtml(rock.id)}">
      <a class="rock-photo-link" href="${detailUrl}" target="_blank" rel="opener" aria-label="${escapeHtml(rock.name)}${text.details}">
    ...
    <div class="rock-details" id="rock-details-${escapeHtml(rock.id)}" data-rock="${escapeHtml(rock.id)}" hidden>
      ...
      <a class="secondary-button rock-detail-link" href="${detailUrl}" target="_blank" rel="opener">${text.details}</a>
    </div>
  `;
}
      ...
      <ul class="catchment-list">
        ${rows.map(({ key, share, entry }) => `
          <li>
            ...
            <a class="catchment-lithology" href="${lithologyUrl(key)}" target="_blank" rel="opener">${escapeHtml(entry.lithology)}</a>
          </li>
        `).join("")}
      </ul>
    </article>
...
function geologyCard(legend, isPoint, area) {
  ...
  return `
    <article class="geology-card${isPoint ? " geology-card-point" : ""}">
      ...
      ${entry ? `<a class="secondary-button geology-detail-link" href="${lithologyUrl(legendKey(legend))}" target="_blank" rel="opener">${text.lithologyDetails}</a>` : ""}
