...
function catalogLink(name, className = "", self = null) {
  ...
  return `<a class="${className} catalog-link" href="${href}" target="_blank" rel="opener">${label}</a>`;
}
...
function catalogRow({ name, text, target, self, withPlaceholder }) {
  ...
  if (rock) {
    ...
    thumb = `<a class="confuse-thumb" href="stone.html?id=${encodeURIComponent(rock.id)}" target="_blank" rel="opener" tabindex="-1" aria-hidden="true"><img src="${catalogEscape(photo.src)}" alt="" loading="lazy"></a>`;
    if (photo.credit) {
  ...
  const nameHtml = target
    ? `<a class="catalog-link" href="${CATALOG_PAGES[target.type]}?id=${encodeURIComponent(target.id)}" target="_blank" rel="opener">${catalogEscape(name)}</a>`
...
function stoneTile(rock) {
  const photo = stonePhoto(rock);
  return `
    <a class="stone-tile" href="stone.html?id=${encodeURIComponent(rock.id)}" target="_blank" rel="opener">
...
//
// 詳細ページは地図から target="_blank" rel="opener" で開くので、そのタブには履歴がない。
// ここで index.html を開くと、そのタブに**もう1枚**地図が出てしまい、
// もとの地図（選んだ地点も結果も残っている）は裏のタブに置き去りになる。
// 新しいタブで開かれたなら、そのタブを閉じてもとの地図に戻す。
//
// 「このタブは別のページから開かれたか」の判定には window.opener を使う。
// 注意: いまのブラウザは target="_blank" に既定で noopener を付ける（Chrome 88 以降）ので、
// rel を書かないだけでは opener は null のままになる。だから自分のページへのリンクには
// rel="opener" を明示してある（同じオリジンの自分のページなので問題ない）。
// history.length は当てにしない。空のタブから開くと、移動していなくても2になる。
function bindBackButton() {
  ...
  button.addEventListener("click", () => {
    // 別のページから開かれたタブ。閉じれば、もとの地図がそのまま前に出る。
